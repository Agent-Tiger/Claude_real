...
const contentLines = [
  {
    type: 'quote',
    text: '"There is nothing impossible to him who will try" (Alexander the Great)'
  },
  { text: '' },
...
/* ── Typewriter hook ────────────────────────────────── */
const TYPE_DELAY = 29
const LINE_PAUSE = 136
const SECTION_PAUSE = 255
...
function useTypewriter(lines) {
  ...
  useEffect(() => {
    ...
    // Determine the string to type for this line
    let textToType = ''
    if (line.text !== undefined) {
      textToType = line.text
    } else if (line.type === 'experience') {
      textToType = `${line.role} @ ${line.place}`
    } else if (line.type === 'education') {
      textToType = 'b.b.a. — canfield business honors' // Primary line to type out
    } else if (line.type === 'tag-grid' || line.type === 'spacer') {
      textToType = '' // These appear instantly relative to their internal turn
    }

    // Special case for completely empty line-spacer lines
    if (line.text === '' && !line.type) {
      const t = setTimeout(() => {
    ...
    // Typical typewriter logic
    if (charIndex < textToType.length) {
      const t = setTimeout(() => {
        setCurrentText(textToType.slice(0, charIndex + 1))
        setCharIndex(p => p + 1)
      }, TYPE_DELAY)
      return () => clearTimeout(t)
    }
  ...
  return { visibleLines, currentText, lineIndex, isDone, showSkip, skip }
}

/* ── Components ────────────────────────────────────── */

function TagGrid({ items, isVisible }) {
...
function Line({ line }) {
  // If it's a simple empty line spacer
  if (line.text === '' && !line.type) return <div className="line-spacer" />

  if (line.type === 'section-label') {
    return <div className="line line-section muted">{line.text}</div>
  }
  ...
  if (line.type === 'tag-grid') {
    return (
      <div className="line">
        <TagGrid items={line.items} isVisible={true} />
      </div>
    )
  }
  ...
  if (line.type === 'quote') {
    return (
      <div className="line quote-container">
...
function App() {
  const { visibleLines, currentText, lineIndex, isDone, showSkip, skip } = useTypewriter(contentLines)
        ...
        <div className="content">
          ...
          {!isDone && (
            <>
              {currentText && (
                <div
                  className={`line ${contentLines[lineIndex]?.type === 'quote' ? '' : 'muted'}`}
                  style={{ opacity: 1, transform: 'none' }}
                >
                  {/* Wrap in quote block if typing the quote line */}
                  {contentLines[lineIndex]?.type === 'quote' ? (
                    <div className="quote-container">
                      <div className="quote-section">
                        <span className="quote-text">{currentText}</span>
                      </div>
                    </div>
                  ) : (
                    currentText
